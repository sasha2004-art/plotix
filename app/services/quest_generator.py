...
def _get_plot_concept_prompt(setting_text: str) -> str:
    """Генерирует высокоуровневый концепт сюжета в виде обычного текста."""
    return f"""
Ты — эксперт-геймдизайнер, известный своими глубокими сюжетами. Твоя задача — написать краткий, но увлекательный концепт для детективного квеста. Опиши его как связный рассказ.
**ТРЕБОВАНИЯ К КОНЦЕПТУ:**
1.  **Формат:** Обычный текст, не JSON.
2.  **Содержание:** Опиши завязку, ключевые этапы расследования, главных персонажей и их мотивы.
3.  **Продвинутые техники (включи в рассказ как минимум ТРИ из списка):**
    - **Ненадежный NPC:** Персонаж, который лжет или имеет скрытые мотивы.
    - **"Замок и Ключ":** Препятствие, которое можно преодолеть, только найдя "ключ" в другой части сюжета.
    - **Ложный след ("Красная селедка"):** Ветка расследования, которая уводит в тупик.
    - **Моральная дилемма:** Сложный выбор без очевидно правильного ответа.
    - **Несколько концовок:** Кратко опиши, как действия игрока могут привести к разным финалам.
**Сеттинг для квеста:**
---
{setting_text}
---
Теперь напиши связный и логичный концепт сюжета, который позже ляжет в основу структуры квеста.
"""

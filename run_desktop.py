...
class Api:
    ...
    def _download_worker(
        ...
        try:
                    ...
                    for chunk in r.iter_content(chunk_size=chunk_size):
                        ...
                        if chunk:
                            ...
                            if current_time - last_update_time >= 1.0 or downloaded_size == total_size:
                                speed = bytes_since_last_update / (
                                    current_time - last_update_time
                                ) if (current_time - last_update_time) > 0 else 0
            ...
            # ИЗМЕНЕНИЕ: Формируем HTML-сообщение с интерактивной ссылкой для ошибки 403
            if status_code == 403:
                model_url = f"https://huggingface.co/{repo_id}"
                message = (
                    f'Для доступа к этой модели необходимо принять ее условия. '
                    f'Пожалуйста, перейдите на <a href="{model_url}" target="_blank" rel="noopener noreferrer">страницу модели</a>, '
                    f'войдите в аккаунт и примите лицензионное соглашение. '
                    f'После этого попробуйте скачать снова.'
                )
            elif status_code == 401:
                message = (
                    f"Ошибка 401: Неверный токен. "
                    "Попробуйте снова войти в Hugging Face через настройки приложения."
                )
